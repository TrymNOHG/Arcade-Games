...
public class Sudoku extends JComponent implements Runnable{
    //I want this class to be able to extract a saved game's information from a file.
    //I, then, want to create a Sudoku.SudokuBoard object based on the game info.
    //Have a save button
    ...
    public Sudoku(int boardLength) {
        ...
        this.sudokuWindow.setSize(700, 700);
        this.sudokuWindow.setTitle("Sudoku");
        this.sudokuWindow.setVisible(true);
        this.sudokuWindow.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
    }

    public void newGame(int boardLength){
        sBoard = new SudokuBoard(boardLength);
        printBoards();
    ...
    protected void paintComponent(Graphics g){
        ...
        Rectangle2D.Double backSquare = new Rectangle2D.Double(100, 100, 90, 1000);
        graphics2D.setColor(Color.black);
        graphics2D.fill(backSquare);
        SudokuGraphics sGraph = new SudokuGraphics(this.boardLength, this.sudokuWindow, mouseLoc);
        sGraph.getNewGrid().genGridArr(graphics2D);
        graphics2D.dispose();
    }
...
//Maybe trying moving things to the paintComponent and using the repaint function.